...
class RosterScheduleSeeder extends Seeder
{
    ...
    // MAPPING NAMA → ID
    //
    // Key   = nama yang dipakai di roster.json
    // Value = ID yang tersimpan di database
    //
    // ID ruangan & kelas HARUS konsisten dengan MasterDataSeeder.
    // ID guru mapel (GT-xxx) dibuat otomatis oleh ensureMasterDataExists().
    // =========================================================================

    /**
     * Kelas: nama di roster.json → ID di master_classes.
     */
    ...
    /**
     * Ruangan: nama di roster.json → ID di master_classrooms.
     * ID HARUS konsisten dengan yang di-seed oleh MasterDataSeeder.
     */
    ...
    /**
     * Mata pelajaran: nama di roster.json → ID di master_subjects.
     * ID HARUS konsisten dengan yang di-seed oleh MasterDataSeeder.
     */
    ...
    /**
     * Guru mapel: nama di roster.json → ID di master_homeroom_teachers.
     * Prefix GT-xxx = Guru Tidak tetap/mapel (bukan wali kelas).
     * Catatan: SRI RIZKI dan SRI RIZKY merujuk orang yang sama → satu ID.
     */
    protected array $teacherNameToId = [
        ...
        'SRI RIZKI'    => 'GT-SRI-RIZKI',   // sama dengan SRI RIZKY di bawah
        'SRI RIZKY'    => 'GT-SRI-RIZKI',   // alias, merujuk orang yang sama
        'SUBRATA'      => 'GT-SUBRATA',
    ...
    /**
     * Hari: nama di roster.json → nama hari di database.
     */
    ...
    /**
     * Siklus minggu: key di roster.json → nilai week_cycle di database.
     */
                ...
                foreach ($days as $dayKey => $slots) {
                    ...
                    foreach ($slots as $slot) {
                        ...
                        $schedules[] = [
                            ...
                            'end_time'              => $endTime,
                            'subject_id'            => $this->resolveSubjectId($slot['mapel']),
                            'teacher_id'            => $this->resolveTeacherId($slot['guru']),
                            'classroom_id'          => $this->resolveClassroomId($slot['ruang']),
                            'period_duration_hours' => (string) ($jpSelesai - $jpMulai + 1),
                            'created_at'            => now(),
                            'updated_at'            => now(),
                        ];
                    }
    ...
    // =========================================================================
    // ENSURE MASTER DATA
    // Membuat data master yang belum ada (dari luar mapping MasterDataSeeder)
    // =========================================================================
    ...
    protected function ensureMasterDataExists(array $kelasList): void
    {
        ...
        foreach ($kelasList as $kelasData) {
            ...
            $homeroomTeacherId    = $this->resolveHomeroomTeacherId($namaKelas, $kelasData['wali_kelas']);
    ...
    // =========================================================================
    // HELPER METHODS - RESOLVE / LOOKUP
    // =========================================================================

    /**
     * Resolve atau buat wali kelas dan return ID-nya.
     * Prioritas: cari berdasarkan teacher_name, jika tidak ada buat baru dengan ID 'HR-{SAFENAME}'.
     */
    protected function resolveHomeroomTeacherId(string $namaKelas, string $waliKelas): ?string
    {
        ...
        $teacherId = 'HR-' . strtoupper(preg_replace('/[^a-zA-Z0-9]/', '-', $waliKelas));
    ...
    // =========================================================================
    // HELPER METHODS - PERHITUNGAN WAKTU
    // =========================================================================

    /**
     * Hitung start_time dan end_time berdasarkan hari dan nomor JP.
     * Waktu JP HARUS sinkron dengan data di MasterScheduleSeeder.
     */
    ...
    /**
     * Tabel alokasi waktu per JP berdasarkan hari.
     *
     * Senin  : Upacara 07:15-08:00, 10 JP, Istirahat 1: 10:00-10:20, Istirahat 2: 12:20-13:00
     * Jumat  : 6 JP, Istirahat 1: 09:30-09:45
     * Reguler: Selasa/Rabu/Kamis, 10 JP, Istirahat 1: 10:10-10:30, Istirahat 2: 12:30-13:10
     */
    ...
    protected function inferGradeLevelAndMajor(string $namaKelas): array
    {
        $gradeLevel = match (true) {
            str_starts_with($namaKelas, 'XII ') => 'XII',
            str_starts_with($namaKelas, 'XI ')  => 'XI',
            default                              => 'X',
        };

        $major = match (true) {
            str_contains($namaKelas, 'TJK TELEKOMUNIKASI') => 'TJK-TELEKOMUNIKASI',
            str_contains($namaKelas, 'PPL GIM')            => 'PPL-GIM',
            str_contains($namaKelas, 'TBKR')               => 'TBKR',
            str_contains($namaKelas, 'T-TEP')              => 'T-TEP',
            str_contains($namaKelas, 'TKR')                => 'TKR',
            str_contains($namaKelas, 'TKJ')                => 'TKJ',
            str_contains($namaKelas, 'TSM')                => 'TSM',
            str_contains($namaKelas, 'RPL')                => 'RPL',
            str_contains($namaKelas, 'TO')                 => 'TO',
            default                                        => 'UMUM',
        };
    ...
    /**
     * Tentukan tipe dan nama tampilan ruangan berdasarkan ID dan nama.
     */
    protected function inferRoomTypeAndName(string $roomName, string $roomId): array
    {
        $type = match (true) {
            str_contains($roomId, 'LAB')      => 'Laboratory',
            str_contains($roomId, 'BKL')      => 'Laboratory',
            str_contains($roomId, 'AXIOO')    => 'Laboratory',
            str_contains($roomId, 'MUSHOLLAH')=> null,
            default                           => 'Theory',
        };
