...
interface CreateBatchHomeroomTeacherModalProps {
    isOpen: boolean;
    setIsOpen: (isOpen: boolean) => void;
}

export function CreateBatchHomeroomTeacherModal({ isOpen, setIsOpen }: CreateBatchHomeroomTeacherModalProps) {
    ...
    const handleDownloadTemplate = () => {
        window.location.href = '/homeroom-teachers/template';
    };
            ...
            <DialogContent className="w-[95vw] max-w-md rounded-xl p-4 sm:p-6 overflow-hidden">
                <form onSubmit={submit} className="flex flex-col gap-6">
                    <DialogHeader className="text-left space-y-2">
                        <DialogTitle className="text-xl font-semibold">Import Guru / Wali Kelas</DialogTitle>
                        <DialogDescription className="text-sm text-muted-foreground leading-relaxed">
                            Tambahkan data guru secara massal menggunakan file Excel (.xlsx).
                        </DialogDescription>
                    </DialogHeader>
