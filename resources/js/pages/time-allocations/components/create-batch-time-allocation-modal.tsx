...
interface CreateBatchTimeAllocationModalProps {
    isOpen: boolean;
    setIsOpen: (isOpen: boolean) => void;
}

export function CreateBatchTimeAllocationModal({ isOpen, setIsOpen }: CreateBatchTimeAllocationModalProps) {
    ...
    const handleDownloadTemplate = () => {
        window.location.href = '/time-allocations/template';
    };
            ...
            <DialogContent className="w-[95vw] max-w-md rounded-xl p-4 sm:p-6 overflow-hidden">
                <form onSubmit={submit} className="flex flex-col gap-6">
                    <DialogHeader className="text-left space-y-2">
                        <DialogTitle className="text-xl font-semibold">Import Alokasi Waktu</DialogTitle>
                        <DialogDescription className="text-sm text-muted-foreground leading-relaxed">
                            Tambahkan jadwal waktu secara massal menggunakan file Excel (.xlsx).
                        </DialogDescription>
                    </DialogHeader>
